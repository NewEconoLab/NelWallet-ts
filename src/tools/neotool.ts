...
namespace wallet.tools
{
    ...
    export class NeoUtil
    {
        ...
        {
            let result: wallet.entity.result = { err: false, result: { pubkey: Uint8Array, prikey: Uint8Array, address: "" } };
         ...
         * nep2FromWif
         */
        public static nep2FromWif(wif: string, password: string): wallet.entity.result
        {
            var prikey: Uint8Array;
            var pubkey: Uint8Array;
            var address: string;
            let res: wallet.entity.result = { err: false, result: { address: "", nep2: "" } };
            try
            {
                prikey = ThinNeo.Helper.GetPrivateKeyFromWIF(wif);
                var n = 16384;
         ...
         * nep2TOWif
         */
        public static async nep2ToWif(nep2: string, password: string): Promise<wallet.entity.result>
        {
            var prikey: Uint8Array;
            var pubkey: Uint8Array;
            var address: string;
            let promise: Promise<wallet.entity.result> = new Promise((resolve, reject) =>
            {
                let n: number = 16384;
                var r: number = 8;
                var p: number = 8
         ...
         * nep6Load
         */
        public static async nep6Load(wallet: ThinNeo.nep6wallet, password: string)
        {
            ...
            try
            {
                ...
                for (let keyindex = 0; keyindex < wallet.accounts.length; keyindex++)
                {
                    ...
                    try
                    {
                        let result: wallet.entity.result = await NeoUtil.getPriKeyfromAccount(wallet.scrypt, password, account);
